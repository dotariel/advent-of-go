...
func NewHand(input string) Hand {
	...
	for _, s := range strings.Split(input, "") {
		if _, ok := hand.cardMap[s]; !ok {
			hand.cardMap[s] = make([]Card, 0)
		}

		hand.cardMap[s] = append(hand.cardMap[s], Card(s))

		hand.Cards = append(hand.Cards, Card(s))
	}
...
func (h Hand) Rank() int {
	...
	if h.cardMap.HasSet() && h.cardMap.HasPair() {
